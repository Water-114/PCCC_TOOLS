...
## Trước khi migrate lần đầu sang PostgreSQL (staging/production)
...
1. Xác nhận đã có backup/snapshot gần nhất của database (Render Postgres có
   ...
   liệu thật).
2. Xác nhận `DATABASE_URL` trên web service đang trỏ đúng database dự định
   (staging hay production) — dán nhầm có thể migrate nhầm database.
...
- **Chưa diễn tập trên PostgreSQL thật** (staging/production) — migration
  `flask db upgrade` đầu tiên trên `pccc-trolynghiepvu-db` sẽ là lần chạy
  thật đầu tiên trên Postgres; cần thực hiện đúng theo checklist "Trước khi
  migrate lần đầu" ở trên, có xác nhận rõ ràng của chủ dự án trước khi chạy.
