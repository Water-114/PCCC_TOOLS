"""Batch 5A Pha 3 Buoc 4 — test route POST /api/aiho/export-bao-cao-tham-dinh:
...
+ dung chu so huu phien. Y het khuon test_aiho_export_cong_van_huong_dan_route.py."""
...
def _register_login_and_grant(client, email="bctd1@pccc.local", password="matkhau123", amount=5):
    ...
    return [
        {
            ...
            "tong_ket": "Hệ thống báo cháy đạt yêu cầu cơ bản.",
            "kien_nghi": {
...
def test_requires_login(client):
    resp = client.post("/api/aiho/export-bao-cao-tham-dinh", json={"session_id": 1, "hang_muc": _valid_hang_muc()})
    assert resp.status_code == 401


def test_missing_session_id_returns_400(client):
    token, _ = _register_login_and_grant(client, email="bctd2@pccc.local")
    resp = client.post(
        "/api/aiho/export-bao-cao-tham-dinh",
...
def test_missing_hang_muc_returns_400(client):
    token, _ = _register_login_and_grant(client, email="bctd3@pccc.local")
...
def test_non_dict_root_returns_400(client):
    token, _ = _register_login_and_grant(client, email="bctd4@pccc.local")
    resp = client.post(
        "/api/aiho/export-bao-cao-tham-dinh",
        json=[1, 2, 3],
        headers={"Authorization": f"Bearer {token}"},
...
def test_other_users_session_returns_404(client):
    token1, _ = _register_login_and_grant(client, email="bctd5a@pccc.local")
    session_id = _open_session(client, token1)

    token2, _ = _register_login_and_grant(client, email="bctd5b@pccc.local")
    resp = client.post(
        "/api/aiho/export-bao-cao-tham-dinh",
...
def test_valid_payload_returns_200_with_downloadable_docx(client):
    token, _ = _register_login_and_grant(client, email="bctd6@pccc.local")
    session_id = _open_session(client, token)
    resp = client.post(
        "/api/aiho/export-bao-cao-tham-dinh",
        json={"session_id": session_id, "hang_muc": _valid_hang_muc()},
...
def test_uses_saved_quy_mo_for_mergefields(client):
    token, _ = _register_login_and_grant(client, email="bctd7@pccc.local")
    session_id = _open_session(client, token)
...
def test_export_does_not_consume_aiho_quota(client):
    token, user_id = _register_login_and_grant(client, email="bctd8@pccc.local")
    session_id = _open_session(client, token)
