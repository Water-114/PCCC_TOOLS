const BASE_URL = import.meta.env.VITE_API_BASE_URL || "http://localhost:5000";

async function request(path, options) {
  const res = await fetch(`${BASE_URL}${path}`, options);
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    throw new Error(data.error || `Lỗi ${res.status}`);
  }
  return data;
}

function postJSON(path, body) {
  return request(path, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
}

export function calculateWaterTank(payload) {
  return postJSON("/api/water/calculate", payload);
}

export function requestAiComment(provider, result) {
  return postJSON("/api/ai/comment", { provider, result });
}

export function getThamDinhOccupancies() {
  return request("/api/tham-dinh/occupancies");
}

export function evaluateThamDinh(payload) {
  return postJSON("/api/tham-dinh/evaluate", payload);
}
